...
void putull(unsigned long long val) { put_unsigned(val, 10, g_HexLower, 0, false); }

void putx(unsigned long long val)   { put_unsigned(val, 16, g_HexLower, 0, false); }
void putX(unsigned long long val)   { put_unsigned(val, 16, g_HexUpper, 0, false); }

void puto(unsigned long long val)   { put_unsigned(val, 8, g_HexLower, 0, false); }

void putb(unsigned long long val)   { put_unsigned(val, 2, g_HexLower, 0, false); }

void putp(const void *ptr)
{
   puts("0x");
   if (sizeof(ptr) == 8)
      put_unsigned((unsigned long long)(uintptr_t)ptr, 16, g_HexLower, 16, true);
   else
