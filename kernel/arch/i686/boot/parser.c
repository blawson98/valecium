...
// Pre-kernel Multiboot v1 -> BOOT_Info translator. Called from entry.S
// before heap/BSS are available; uses only static storage and hand-written
// loops. Single file-scope BOOT_Info (s_bootInfo) is explicitly zeroed.
...
// Pre-heap storage for parsed boot parameters. Explicitly zeroed inside
// Parser_Multiboot rather than relying on BSS.
static BOOT_Info s_bootInfo;

// Copy at most (max_len - 1) bytes from a physical-address C string into
// dst[], always null-terminating. Safe pre-heap.
static void CopyString(char *dst, const char *src, uint32_t max_len)
{
   uint32_t i = 0;
...
// Portable byte-loop memset substitute; avoids kernel memory subsystem deps.
...
// Translate Multiboot v1 info into BOOT_Info, then hand control to start().
// Called (cdecl) from entry.S with magic in eax, mbi pointer in ebx.
